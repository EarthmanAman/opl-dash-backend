...
class CreateSaleView(ListCreateAPIView):
    ...
    queryset = Sale.objects.all().select_related().cache()
...
class UploadExcel(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        result = sale_resource.import_data(dataset, dry_run=True)  # Test the data import
        if not result.has_errors():
            # sale_resource.import_data(dataset, dry_run=False)  # Actually import now
            return Response({"status":status.HTTP_201_CREATED, "message":"Upload successful"})
        return Response({"status":status.HTTP_403_FORBIDDEN, "message":"Check your file format. If error persist contact admin"})